...
*   **修行の記録機能**: ローカル保存によるハイスコアランキングに対応。
*   **隠し要素**: 特定の条件で解放される高難易度モード。
*   **多言語対応**: 日本語と英語の表示切り替え（利用規約、チュートリアル等含む）に対応。
*   **カスタマイズ設定機能**: 各自の環境や好みに合わせて、BGMのON/OFFやモバイル向け操作モード（スライド複数種、ジョイコン）の設定が可能。
*   **ブッダの声**: 修行を進めることで、仏教の教え（中道、十二縁起など）を学ぶことができるメッセージ機能。
...
### スマートフォン（タッチ操作）
*   「環境設定」メニューから、プレイスタイルに合わせて「スライド1」「スライド2」「ジョイコン」などいくつかの操作モードが選択できます。
*   **移動**: 画面のスワイプや仮想ジョイコンによる移動（設定により変化）
*   **ショットボタン**: 読経
*   **必殺技ボタン**: 必殺技発動
...
本ゲームはブラウザのみで動作するフロントエンド完結型ですが、以下のセキュリティおよび不正対策を施しています。

*   **XSS（クロスサイトスクリプティング）対策**: ローカルストレージ内のデータを画面に描画（ランキング表示等）する際、HTML特殊文字を常にエスケープ（無害化）しています。
*   **セーブデータの難読化**: スコアや開放レベルなどの `localStorage` へ保存されるデータは、直感的な改ざんを防ぐためBase64による簡易的な難読化処理を行って保存しています。
*   **パスワードハッシュ化**: 隠しコマンド等で使用するパスワードは、ソースファイル内に平文で保持せず、ハッシュ値に変換して比較する設計としています。
*   **API連続アクセス防止**: 訪問者カウンター取得時に `sessionStorage` を用いて、1回のブラウザセッションにつき1度のみカウントアップのリクエストを送信するよう制御しています。
