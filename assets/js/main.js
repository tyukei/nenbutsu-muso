        ...
        function isLevelUnlocked(level) {
            ...
            if (level === 'demon') return cleared.includes('hard'); // 鬼レベルはLev3クリアで解放
            return false;
        }
        ...
        function checkPassword() {
            ...
            if (password === 'nenbutsu-mashimashi1') {
                ...
                // 鬼レベルを解放（easy, normal, hardをクリア済みにする）
                if (!cleared.includes('easy')) {
                    cleared.push('easy');
                }
                if (!cleared.includes('normal')) {
        ...
        function gameOver(win) {
            ...
            const settings = levelSettings[currentLevel];
            const targetDisplay = settings.isInfinite ? '∞' : targetScore;
            const levelValue = settings.isInfinite ? '鬼' : ({
                easy: '1',
                normal: '2',
                hard: '3'
            ...
            if (win) {
                ...
                if (settings.isInfinite) {
                    document.getElementById('resultText').textContent = `鬼の煩悩に呑まれてしまいました...`;
                } else {
