...
このドキュメントは、このプロジェクト「煩悩シューティング」の開発を支援するAI（Gemini）のためのガイドラインです。

## プロジェクト概要

「煩悩シューティング」は、修行僧が迫り来る「煩悩」を「読経」で撃ち落とす、ブラウザベースのシューティングゲームです。PCとモバイルの両方に対応しており、難易度設定やランキング機能を備えています。
...
### 5. データ永続化
*   **修行の記録**: `localStorage` (`nenbunRankings`)。
...
5.  **セキュリティ対策**:
    *   **XSS防止**: ユーザーの入力・各種変数などを `innerHTML` で描画する場合は、必ず `escapeHtml()` を通して無害化してください。
    *   **データの難読化**: `localStorage` で保存するスコアや進捗データ等は `btoa`/`atob` を用いて簡易的な難読化と復号処理を挟んでください。
    *   **パスワードの隠蔽**: コード上にパスワード等を記載する場合は平文を避け、ハッシュ化して比較判定を行ってください。
